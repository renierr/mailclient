...
- Frontend: **QML (QtQuick + QtQuick.Controls)**, sources in `qml/`. HTML mail rendered via `QtWebEngine`.
...
- Run `cargo fmt`, `cargo clippy -- -D warnings`, and `cargo test -p mailcore` before finishing a backend change. Run `qmllint`/`qmlformat` (from `/usr/lib/qt6/bin`) on changed QML when available.
...
- QML: one component per file in `qml/`, `PascalCase.qml` filenames, `qmllint`-clean, no inline JS business logic beyond formatting. All user-visible strings ready for `qsTr()`.
