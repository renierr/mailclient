...
```text
qml/  ──QtQuick UI──▶  crates/mailapp (cxx-qt bridge, Qt models) ─▶ crates/mailcore (db/store/sync/queue)
Bay                                               ▲
...
- `crates/mailapp`: `cxx-qt` QObject bridge (`Bridge`, `AccountListModel`, `FolderTreeModel`, `MessageListModel`, composer controller) + `main.rs` loading `qml/Main.qml`.
- `qml/`: `Main.qml`, `Sidebar.qml`, `MessageList.qml`, `MessageView.qml`, `Composer.qml`, `AccountSetup.qml`, `Settings.qml`, `components/*`.
...
```sh
./scripts/dev.sh            # debug build + run (needs Qt6 + qml in QML2_IMPORT_PATH if run headless)
...
qmllint qml/*.qml qml/components/*.qml  # QML lint (uses /usr/lib/qt6/bin when on PATH)
```
