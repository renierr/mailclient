...
#[tokio::test]
async fn discovery_skips_noselect_merges_lsub_and_keeps_first_role() {
    let server = MockImapServer::start("IMAP4rev1", |tag, rest| {
        let upper = rest.to_ascii_uppercase();
        if upper.starts_with("LIST") {
            // Roots for the subtree pass carry no names the other passes miss.
            if rest.contains('%') {
                return vec![
                    "* LIST (\\HasNoChildren) \"/\" INBOX\r\n".to_string(),
                    format!("{tag} OK LIST completed\r\n"),
    ...
    let by_path: std::collections::HashMap<_, _> =
        found.into_iter().map(|f| (f.path, f.role)).collect();
    assert!(!by_path.contains_key("Ghosts"), "noselect must be skipped");
    assert!(
        by_path.contains_key("LsubOnly"),
        "lsub-only folder must merge"
    );
    assert_eq!(
        by_path.get("Sent"),
        Some(&crate::models::FolderRole::Sent),
        "first pass (SPECIAL-USE attr) wins over the bare LSUB re-see"
    );
}
