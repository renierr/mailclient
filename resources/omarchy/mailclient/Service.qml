...
Item {
    ...
    function displayError(msg) {
        ...
        if (text === "")
            return "";
        if (isTransientError(text) && (root.knownUnread < 0 || root.failStreak < 2))
            return "";
        return text;
    }
    ...
    function refreshCache() {
        if (syncProcess.running || statusProcess.running || root.syncing)
            return;
    ...
    function applyStatus(raw) {
        ...
        if (knownUnread < 0 || parsed.unread < knownUnread)
            knownUnread = parsed.unread;
    }
    ...
    Process {
        ...
        onExited: function (exitCode) {
            root.syncing = false;
            if (exitCode === 0)
                root.applyReport(syncStdout.text);
            else {
                root.failStreak += 1;
